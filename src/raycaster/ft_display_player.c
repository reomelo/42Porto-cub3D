...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/10/12 15:39:41 by riolivei          #+#    #+#             */
	...
	while (++i < map->n_lines)
	{
		...
		while (++j < map->n_col)
		{
			if (map->map_arr[i][j] == '1' || map->map_arr[i][j] == '-')
				mlx_put_image_to_window(mlx->mlx, mlx->win, map->imgs[0].img,
					j * SQUARE_SIZE, i * SQUARE_SIZE);
			else
				mlx_put_image_to_window(mlx->mlx, mlx->win, map->imgs[1].img,
					j * SQUARE_SIZE, i * SQUARE_SIZE);
		}
	}
	mlx_put_image_to_window(mlx->mlx, mlx->win, map->imgs[2].img,
		(player->x - 0.25) * SQUARE_SIZE, (player->y - 0.25) * SQUARE_SIZE);
